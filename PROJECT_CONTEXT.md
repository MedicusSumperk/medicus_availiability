...
- `TYP` - appointment type, recent rows commonly use `1`
- `PRISEL` - attendance/status flag, recent rows commonly use `N`
- `INFO` - short text field, useful for rollback-test marker
- `DATZAPIS` - write date, recent rows use current date
- `CREATEDBY` - creator user ID, recent rows showed `10`
- `CREATED` / `CHANGED` - populated by triggers

Used to mark generated theoretical slots as occupied. This is also the current candidate direct booking/write target. Phase 3 still must verify the correct production write path, field semantics, and UI visibility before any production booking script is created.
...
`scripts/tests/` contains read-only validation and inspection scripts used to verify the database pipeline plus Phase 3 mapping scripts.
...
## Run Phase 3 Rollback Insert Test
...
- start time
- duration minutes
- `OBJOBJ.TYP`, default `1`
- `CREATEDBY`, default `10`

It inserts a row with `INFO = AI_RECEPTION_TEST_ROLLBACK`, reads the generated `IDOBJ` back in the same transaction, prints the row, then calls `rollback()`. It does not commit.
...
- Direct `INSERT` into `OBJOBJ` appears plausible for the initial rollback test.
- `IDOBJ` is generated by trigger `OBJOBJ_BI` through generator `GEN_IDOBJ`.
- `CREATED` and `CHANGED` are populated by triggers.
- Insert/update/delete triggers call sync/log behavior such as `COS_NEEDCALSYNC` and `sp_log`, so rollback testing is important before any commit.
- Recent appointment rows show a minimal practical pattern with `IDPAC`, `IDPRAC`, `DATUM`, `CAS`, `TYP`, `PRISEL`, `IDUZI`, `DATUMDO`, `CASDO`, `DATZAPIS`, and `CREATEDBY`.

Open questions:

- Is direct `INSERT` into `OBJOBJ` sufficient for production, or should booking use a stored procedure?
- Which patient identifier fields should be copied/filled for best UI display?
- Which appointment type/status/source fields are required for the appointment to appear correctly in the Medicus UI?
- Is `INFO` the right short marker field, or should longer notes use `POZNAMKA`?
...
The first implementation defaults to rollback. A committed test should happen only after the write path and test data are explicitly agreed with the client.
...
The core scheduling logic is implemented and validated. Phase 2 weekly CLI was tested on the Windows server and generates usable report files while keeping database operations read-only. Phase 3 has a rollback-only insert test ready for controlled execution with client-approved test data. The main remaining Phase 2 validation item is confirming the correct source of truth for `TYPTYD`.
