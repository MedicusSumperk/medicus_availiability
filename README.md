...
`/patient-lookup` finds patient candidates in `KAR`, supports phone/name/date/full `RODCIS` lookup, verifies identity with the last 4 digits of `RODCIS`, and returns future `OBJOBJ` appointments only after verification. It is read-only.
